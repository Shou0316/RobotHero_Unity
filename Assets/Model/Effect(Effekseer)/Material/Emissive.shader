...
Shader "EffekseerMaterial/Emissive" {
...
Properties{
	...
	_Texture_Tex("Color (RGBA)", 2D) = "white" {}
...
float4 val6 = ConvertFromSRGBTexture(tex2Dlod(_Texture_Tex,float4(GetUV(uv1),0,0)));
float3 val1=val6.xyz;
float4 val7=Emissive;
float4 val8=(val7*val6);
...
float3 emissive = float3(val8.x,val8.y,val8.z);
...
float4 val6 = ConvertFromSRGBTexture(tex2D(_Texture_Tex,GetUV(uv1)));
float3 val1=val6.xyz;
float4 val7=Emissive;
float4 val8=(val7*val6);
float3 normalDir = float3(0.5,0.5,1.0);
